# app.py — Streamlit Cloud (PAT + REST, SSE/JSON 자동, v2 Statements, 영구 히스토리)
...
# ---------------------------
# Secrets (Streamlit Cloud: Settings → Secrets에 아래 키 필요)
# [snowflake]
# account_base = "https://<org>-<acct>.snowflakecomputing.com"   # regionless (Snowsight URL)
# sql_base     = "https://<locator>.snowflakecomputing.com"       # regional  (CURRENT_ACCOUNT() 기반)
...
def parse_events_response(events: list):
    ...
    for ev in events:
        ...
        if ev.get("event") != "message.delta":
            # 대부분 delta 누적으로 충분 (message.completed 는 무시)
            continue
        payload = ev.get("data", {})
        ...
        for item in content:
            ...
            # {'type':'tool_results', ...}
                ...
                for r in item.get("tool_results", {}).get("content", []):
                    _pull_from_tool_result(r, citations, set_sql)
            # tool_use 는 스킵 (곧이어 tool_results 도착)
    return _normalize_text("".join(text_parts)), sql_holder["v"], citations
...
# ---------------------------
# 폴백(SQL 질문인데 text 비었고 SQL만 있을 때 간단 메시지 보강)
# ---------------------------
if not text and sql:
    text = "Generated SQL below. Executing and summarizing the result…"
